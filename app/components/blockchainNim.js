...
class Blockchain extends React.Component {
  ...
  async setValue(e) {
    ...
    const value = parseInt(this.state.valueSet, 10);

    const toSend = SimpleStorage.methods.set(value);
    try {
      const gas = await toSend.estimateGas();
      this.setState({mining: true});
      this._addToLog("SimpleStorage.methods.set(value).send()");
      await toSend.send({gas: gas + 200});
    ...
    return (<React.Fragment>
        ...
        <Form onKeyDown={(e) => this.checkEnter(e, this.setValue)}>
          <FormGroup className="inline-input-btn">
            ...
            {this.state.setTo !== '' && <p>Value set to: {this.state.setTo}</p>}
